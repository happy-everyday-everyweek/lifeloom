...
核心 M2 权限已完成（权限为独立系统插件）：非系统插件的跨边界操作（访问他方机制状态、调用他方钩子）经核心极薄闸门转发给权限插件；权限插件负责放行决策与用户提示；未装权限插件时一律拒绝（fail-closed）。连同 M0 运行骨架、M1 热替换与 M3 首批原语（事件总线、参数存储），桌面端可演示全流程，29 个单元测试覆盖；Android 外壳骨架可构建 APK 并随包携带核心库。底层系统插件层整体设计已定稿（草案 v0.3，14 个成员，见 [docs/底层插件设计.md](docs/底层插件设计.md)）；M3 继续推进——单逻辑线程与外壳完善随后。
