...
核心 M2 权限已完成（权限为独立系统插件）：非系统插件的跨边界操作（访问他方机制状态、调用他方钩子）经核心极薄闸门转发给权限插件；权限插件负责放行决策与用户提示；未装权限插件时一律拒绝（fail-closed）。连同 M0 运行骨架、M1 热替换，桌面端可演示全流程，21 个单元测试覆盖；Android 外壳骨架可构建 APK 并随包携带核心库。调度与参数存储（M3）在后续里程碑中加入。
